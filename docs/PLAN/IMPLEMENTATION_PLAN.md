...
- 全モデルローダー (`core/model_factory.py`) における早期サイズ計算とメモリ管理の強化
    - ロード前にモデルサイズをキャッシュ/設定/計算から特定
    - 計算したサイズを設定ファイル (`annotator_config.toml`) へ自動保存
    - サイズに基づいた事前メモリチェックによるロード中断機能
    - サイズに基づいたキャッシュクリア判断の改善
...
## 6. 改訂履歴
- 1.1.1 (2024-04-19): 全モデルローダーの早期サイズ計算・メモリ管理強化
