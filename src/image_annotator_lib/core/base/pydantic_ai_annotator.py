...
from typing import Any, ClassVar, Sequence, Self, Union, cast
import hashlib
import json
...
class AnnotationAgentConfig:
    ...
    # 基本設定
    model_id: Union[KnownModelName, str]
    name: str
...
class AdvancedAgentFactory:
    ...
    def create_optimized_agent(
        ...
        # 設定変更チェック
        if (cache_key in cls._agent_cache and 
            cls._config_hashes.get(cache_key) == config_hash):
...
class PydanticAIWebAPIAnnotator(BaseAnnotator):
    ...
    async def _predict_batch(
        ...
        for i in range(0, len(images), batch_size):
            ...
            # 結果処理
            for result, (_, phash) in zip(batch_results, batch_tasks):
