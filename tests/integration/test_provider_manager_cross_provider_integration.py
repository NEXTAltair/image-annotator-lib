...
class TestProviderManagerCrossProviderIntegration:
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.get_cached_agent') as mock_get_agent:
            ...
            def mock_agent_creation(model_name, api_model_id, api_key, config_data=None):
                ...
                if "openai" in model_name or "gpt" in api_model_id:
                    mock_agent.run.return_value = MagicMock(data={"tags": ["openai_tag"], "provider": "openai"})
                elif "anthropic" in model_name or "claude" in api_model_id:
                    mock_agent.run.return_value = MagicMock(data={"tags": ["anthropic_tag"], "provider": "anthropic"})
                elif "google" in model_name or "gemini" in api_model_id:
                    mock_agent.run.return_value = MagicMock(data={"tags": ["google_tag"], "provider": "google"})
                else:
                    mock_agent.run.return_value = MagicMock(data={"tags": ["unknown_tag"], "provider": "unknown"})
            ...
            for model_name in test_models:
                try:
                    ...
                    pytest.fail(f"Sequential multi-provider test failed for {model_name}: {str(e)}")
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.get_cached_agent') as mock_get_agent:
            ...
            for model_name, api_model_id in test_scenarios:
                try:
                    ...
                    pytest.fail(f"Concurrent multi-provider test failed for {model_name}: {str(e)}")
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.get_cached_agent') as mock_get_agent:
            ...
            def mock_agent_creation(model_name, api_model_id, api_key, config_data=None):
                ...
                if agent_key not in shared_agents:
                    mock_agent = MagicMock()
                    mock_agent.run.return_value = MagicMock(
                        data={"tags": [f"shared_tag"], "calls": len(agent_creation_calls)}
                    )
            ...
            expected_shared_agents = len(set((req[1], multi_provider_configs["openai_model"]["api_key"]) 
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.get_cached_agent') as mock_get_agent:
            ...
            for model_name, api_model_id in switching_sequence:
                try:
                    ...
                    pytest.fail(f"Provider switching test failed at {model_name}: {str(e)}")
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.get_cached_agent') as mock_get_agent:
            ...
            for model_name, api_model_id, should_succeed in test_sequence:
                try:
                    ...
                    if should_succeed:
                        pytest.fail(f"Provider {model_name} should have succeeded but failed: {str(e)}")
    ...
    def test_openrouter_provider_integration(self, multi_provider_configs, lightweight_test_images):
        ...
        with patch('image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory.create_openrouter_agent') as mock_create_openrouter:
            ...
            try:
                ...
                pytest.fail(f"OpenRouter provider integration failed: {str(e)}")
