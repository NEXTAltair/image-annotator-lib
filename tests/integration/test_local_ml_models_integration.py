...
class TestLocalMLModelsIntegration:
    ...
    def test_onnx_model_loading_integration(self, mock_onnx_session, model_categories):
        ...
        try:
            ...
            pytest.fail(f"ONNX model loading failed for {test_model}: {str(e)}")
    ...
    def test_tensorflow_model_loading_integration(self, mock_tf_load, model_categories):
        ...
        try:
            ...
            pytest.fail(f"TensorFlow model loading failed for {test_model}: {str(e)}")

    @pytest.mark.integration
    @pytest.mark.fast_integration  
    def test_clip_model_loading_integration(self, model_categories, managed_config_registry, lightweight_test_images):
        ...
        try:
            ...
            with patch('image_annotator_lib.core.model_factory.ModelLoad.load_clip_components') as mock_load_clip:
                ...
                mock_head.forward.return_value = MagicMock(detach=MagicMock(return_value=MagicMock(cpu=MagicMock(return_value=MagicMock(numpy=MagicMock(return_value=np.array([0.75])))))))
                
                # Test through the annotate API
                results = annotate(
                    images_list=lightweight_test_images[:1],
                    model_name_list=[test_model]
                )
            ...
            pytest.fail(f"CLIP model integration test failed for {test_model}: {str(e)}")
        ...
        for model_name in test_models:
            with patch('image_annotator_lib.api._create_annotator_instance') as mock_create:
                ...
                try:
                    ...
                    pytest.fail(f"Local ML model inference failed for {model_name}: {str(e)}")
        ...
        for model_name in test_models:
            with patch('image_annotator_lib.api._create_annotator_instance') as mock_create:
                ...
                try:
                    ...
                    pytest.fail(f"Error not properly handled for {model_name}: {str(e)}")
    ...
    def test_memory_management_with_local_models(self, lightweight_test_images):
        ...
        with patch('image_annotator_lib.core.model_factory.ModelLoad') as mock_model_load_class:
            ...
            try:
                ...
                pytest.fail(f"Memory management test failed: {str(e)}")
        ...
        with patch('image_annotator_lib.core.model_factory.ModelLoad.load_model') as mock_load:
            ...
            for test_name, test_image in test_cases:
                try:
                    ...
                    pytest.fail(f"Image preprocessing failed for {test_name}: {str(e)}")
    ...
    def test_concurrent_local_model_loading(self, lightweight_test_images):
        ...
        with patch('image_annotator_lib.core.model_factory.ModelLoad.load_model') as mock_load:
            ...
            try:
                ...
                pytest.fail(f"Concurrent model loading failed: {str(e)}")
        ...
        try:
            ...
            for model_name in found_local_models[:3]:  # Test first 3
                try:
                    ...
                    pytest.fail(f"Model registry integration failed for {model_name}: {str(e)}")
                    
        except Exception as e:
            pytest.skip(f"Model registry test skipped due to initialization issue: {str(e)}")
