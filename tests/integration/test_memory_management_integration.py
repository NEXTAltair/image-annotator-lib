...
class TestMemoryManagementIntegration:
    ...
    def test_provider_instance_sharing_memory_efficiency(
        ...
        with patch(
            ...
            def mock_agent_creation(model_name, api_model_id, api_key, config_data=None):
                ...
                provider_key = (
                    f"{api_key}_{api_model_id.split(':')[0] if ':' in api_model_id else 'openai'}"
                )
    ...
    def test_lru_cache_memory_management(self, memory_test_configs, lightweight_test_images):
        ...
        with patch(
            "image_annotator_lib.core.pydantic_ai_factory.PydanticAIProviderFactory"
        ) as mock_factory:
    ...
    def test_memory_pressure_handling(self, memory_test_configs, lightweight_test_images):
        ...
        with patch(
            "image_annotator_lib.core.model_factory.image_annotator_lib.api._create_annotator_instance"
        ) as mock_load_model:
            ...
            with patch(
                ...
                def mock_model_loading(model_name):
                    concurrent_local_loads.append(
                        {"time": time.time(), "model": model_name, "count": len(concurrent_local_loads) + 1}
                    )
    ...
    def test_mixed_workload_memory_efficiency(self, memory_test_configs, lightweight_test_images):
        ...
        with patch(
            ...
            with patch(
                ...
                resource_usage = {"webapi_agents": 0, "local_models": 0, "peak_webapi": 0, "peak_local": 0}

                def mock_agent_creation(model_name, api_model_id, api_key, config_data=None):
                    resource_usage["webapi_agents"] += 1
                    resource_usage["peak_webapi"] = max(
                        resource_usage["peak_webapi"], resource_usage["webapi_agents"]
                    )
                ...
                def mock_model_loading(model_name):
                    ...
                    resource_usage["peak_local"] = max(
                        resource_usage["peak_local"], resource_usage["local_models"]
                    )
