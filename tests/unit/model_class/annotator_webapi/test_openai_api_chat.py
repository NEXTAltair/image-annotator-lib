...
def annotator():
    with patch(
        "image_annotator_lib.model_class.annotator_webapi.openai_api_chat.config_registry"
    ) as mock_cfg:
...
def test_run_inference_success(annotator):
    ...
    with patch.object(
        annotator, "_call_openrouter_with_json_schema", return_value=DummyResponse(DUMMY_JSON_STR)
    ):
...
def test_run_inference_content_empty(annotator):
    ...
    annotator.api_model_id = DUMMY_MODEL
    annotator._wait_for_rate_limit = MagicMock()
    annotator._call_openrouter_with_json_schema = MagicMock(return_value=DummyResponse(None))
    results = annotator._run_inference([DUMMY_BASE64])
...
def test_run_inference_choices_empty(annotator):
    ...
    annotator.api_model_id = DUMMY_MODEL
    annotator._wait_for_rate_limit = MagicMock()
    annotator._call_openrouter_with_json_schema = MagicMock(return_value=DummyResponseNoChoices())
    results = annotator._run_inference([DUMMY_BASE64])
    assert results[0]["response"] is None
...
def test_run_inference_invalid_json(annotator):
    ...
    annotator.api_model_id = DUMMY_MODEL
    annotator._wait_for_rate_limit = MagicMock()
    annotator._call_openrouter_with_json_schema = MagicMock(return_value=DummyResponse("{invalid json}"))
    results = annotator._run_inference([DUMMY_BASE64])
    assert results[0]["response"] is None
...
def test_run_inference_api_error(annotator):
    ...
    annotator.api_model_id = DUMMY_MODEL
    annotator._wait_for_rate_limit = MagicMock()
    annotator._call_openrouter_with_json_schema = MagicMock(side_effect=Exception("API error!"))
    results = annotator._run_inference([DUMMY_BASE64])
    assert results[0]["response"] is None
