...
def test_all_providers_structure():
    ...
    providers = [
        ("OpenAI", "image_annotator_lib.model_class.annotator_webapi.openai_api_response", "OpenAIApiAnnotator"),
        ("Anthropic", "image_annotator_lib.model_class.annotator_webapi.anthropic_api", "AnthropicApiAnnotator"),
        ("Google", "image_annotator_lib.model_class.annotator_webapi.google_api", "GoogleApiAnnotator"),
        ("OpenRouter", "image_annotator_lib.model_class.annotator_webapi.openai_api_chat", "OpenRouterApiAnnotator"),
    ]

    # 共通の必要メソッド
    required_methods = [
        "__init__", "__enter__", "__exit__", 
        "run_with_model", "_handle_api_error"
    ]
    ...
    try:
        ...
        for api_model_id, expected_provider in test_cases:
            ...
            if determined_provider != expected_provider:
                print(f"❌ プロバイダー判定失敗: {api_model_id} -> 期待:{expected_provider}, 実際:{determined_provider}")
                return False
        
        # ProviderManager必要メソッド確認
        required_manager_methods = [
            "get_provider_instance", "run_inference_with_model", "_determine_provider"
        ]
...
def test_provider_factory_integration():
    ...
    try:
        from image_annotator_lib.core.pydantic_ai_factory import PydanticAIProviderFactory, PydanticAIAnnotatorMixin
        
        # Factory必要メソッド確認
        required_factory_methods = [
            "get_provider", "create_agent", "get_cached_agent", 
            "create_openrouter_agent", "_extract_provider_name"
        ]
        ...
        required_mixin_methods = [
            "__init__", "_preprocess_images_to_binary", "_run_inference_with_model",
            "_setup_agent", "_get_provider_name"
        ]
